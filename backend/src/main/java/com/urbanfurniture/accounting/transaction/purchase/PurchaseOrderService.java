...
public class PurchaseOrderService {
    ...
    public PageResponse<TransactionDtos.OrderResponse> search(String search, OrderStatus status, Pageable pageable) {
        ...
        return PageResponse.of(
                purchaseOrderRepository.search(term, status, currentUser.contactScopeOrNull(), pageable),
                this::toResponse);
    }
    ...
    public TransactionDtos.OrderResponse create(TransactionDtos.OrderRequest request) {
        ...
        PurchaseOrder order = PurchaseOrder.builder()
                .orderNo(documentNumberService.next(DocumentType.PURCHASE_ORDER))
                .contact(vendor)
    ...
    public TransactionDtos.OrderResponse update(Long id, TransactionDtos.OrderRequest request) {
        ...
        if (order.getStatus() != OrderStatus.DRAFT) {
            throw new ApiExceptions.BusinessRuleException(
                    "Only draft purchase orders can be edited (this one is " + order.getStatus() + ")");
        }
    ...
    public TransactionDtos.OrderResponse confirm(Long id) {
        ...
        if (order.getStatus() != OrderStatus.DRAFT) {
            throw new ApiExceptions.BusinessRuleException(
                    "Only a draft purchase order can be confirmed (this one is " + order.getStatus() + ")");
        }
    ...
    PurchaseOrder requireOrder(Long id) {
        return purchaseOrderRepository.findWithLinesById(id)
                .orElseThrow(() -> new ApiExceptions.NotFoundException("Purchase order", id));
    }

    private void applyLines(PurchaseOrder order, List<TransactionDtos.LineRequest> lineRequests) {
    ...
    TransactionDtos.OrderResponse toResponse(PurchaseOrder order) {
        Bill generated = billRepository.findByPurchaseOrderId(order.getId()).stream()
        ...
        return new TransactionDtos.OrderResponse(
                ...
                generated == null ? null : generated.getBillNo());
    }
}
